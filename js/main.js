...
function openModal(title, content, isCalendar = false, link = null) {
    ...
    const modalFooter = document.getElementById('modalFooter'); // Get the footer
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    accordionItems.forEach(item => {
        ...
        if (item.dataset.directLink === 'true') {
            ...
            if (directLinkUrl) {
                // Listener on the *entire item* (section) for direct links, restoring original site behavior
                ...
                item.addEventListener('keydown', function(e) {
                  if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    // 'this' refers to 'item' (the section), allowing consistent behavior
                    this.click();
                  }
                });
            }
        } else { // If it's a regular collapsible accordion
    ...
    function showSlides(slideshowId, n) {
        ...
        let slides = slideshow.container.querySelectorAll('.slide');
